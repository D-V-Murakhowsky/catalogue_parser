...
class ScrapyPageGetter(Spider):
    ...
    def logged_in(self, response):
        if response.url == 'http://b2b.spartakelectronics.com/ru/dashboard':
            file_logger.debug('Logged in successfully')
    ...
    def get_pages_range(self, response):
        if response.url == config.catalogue_url:
            file_logger.debug('Page range is gotten')
    ...
    def get_page(self, response):
        ...
        page_number = int(response.url.split('page=')[1])
        logger.debug(f'Page {page_number} parsed')
        file_logger.debug(f'Page {page_number} parsed')
        self._signals.page.emit(page_number)
